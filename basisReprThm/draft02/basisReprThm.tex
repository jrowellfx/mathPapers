...
\end{align*}
This statement for $n$ only represents the integers 92007, 92107, \dots{} 92907, so let's come up
...
Anyway, it's clear that the first ``non-nine" number, in our case the ``2", got changed
to a ``3" and all the 9's got changed to 0's, and the rest were unchanged
...
Without further adieu, let's get to the actual proofs!
...
And since the digit ``1" is valid in any base $b>1$, this proves the base case.
\end{jprIn}
...
In base-ten $d_0$ can be at most eight, etc.\footnote{Recall
the axiom of ``Distribution'' that is $a(b+c)=ab+ac$}.
\begin{align*}
n &= d_kb^k+d_{k-1}b^{k-1}+\dots+d_2b^2+d_1b^1+d_0b^0 &\text{(Induction Assumption.)}\\
&\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\text{if and only if,}\\
n+1 &= d_kb^k+d_{k-1}b^{k-1}+\dots+d_2b^2+d_1b^1+d_0b^0 + 1 &\text{(Add 1 to both sides.)}\\
&= d_kb^k+d_{k-1}b^{k-1}+\dots+d_2b^2+d_1b^1+d_0b^0 + b^0 &\text{(Restate 1 as }b^0\text{)}.\\
&= d_kb^k+d_{k-1}b^{k-1}+\dots+d_2b^2+d_1b^1 + (d_0+1)b^0 &\text{(Factor out }b^0\text{)}.
\end{align*}
---jpr rewrite---
