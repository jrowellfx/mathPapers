...
So $10^1$ means that there is only one ten multiplied together, or one ``0" following the ``1",
...
then we would always get 499 apples; There is NO other way to split this big pile of apples up
...
Let the term $d$ be one of the digits in our number. So $d$ represents how many lots of some power-of-ten we have,
and $d$ must be between 0 and 9 inclusive, so the contribution of that digit to the integer would be this amount:
...
Our expression should only have one digit for each power-of-ten, in other words our expression should never allow
something like this.
...
which violates our rule of keeping the value of the digit in the range of 0 to 9.
