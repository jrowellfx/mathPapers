...
Furthermore every number raised to the 0\textsuperscript{th} power~is~1.\footnote{Proof: Since $a^{b+c}=a^ba^c$ consider when $c=0$; that is,
...
it's just some number between 0 and 9, times a power-of-ten like each of the other digits.
...
so that once we prove it we may confidently go forward using base-ten with
impunity knowing that we can represent each and every integer with it's own unique label.
...
of which is that it works - perhaps not as elegantly as Mr. Hardy could provide to us, but I'm no G. H. Hardy.
...
So how to prove the theorem?  We're going to prove the theorem by following our
experience with and intuition about about base-ten numbers.
For example, the odometer in your car - as you keep adding one to the 
miles, it keeps making a new base-ten number, it keeps rolling along, making new base-ten numbers, each a new one
and each one with well defined meaning in base-ten.
Each one is a totally valid base-ten number.
We could have an odometer that rolls off binary digits - same idea.  See binary odometer gif.

But that odometer trick is only one possible way tthat we could have made a base-ten number -
there might some magic box that also makes them. Maybe it makes them differently?
Well - let's assume that this magic box does exist and that it made a different base-ten number for the
same integer... then we show that this is impossible.  That proves that there is only one way 
to make them.

So the odometer idea is captured by Mathematical Induciton.
