...
%All the arguments we just made are essentially the same so picking up at equation above, with our new terms, we have:
...
that\footnote{Actually the theorem, the constraint on $b$ only being that $b\ne0$, however to keep the
remainder positive, the restriction on $r$ is stated like this $0 \le r \le (\left|b\right|-1)$.}:
...
In fact here's what another\footnote{besides aforementioned Gauss and Euclid.} of the
world's greatest Mathematicians, ``G.H. Hardy'' had to say about
proofs like this\footnote{coincidentally in an essay he wrote
