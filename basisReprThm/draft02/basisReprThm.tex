...
\[p_1=2<p_2=3<p_3=5<\dots<p_n\]
 is a list of ALL the primes.

Let $P=p_1p_2p_3\dots{}p_n+1$.

Then for any prime $p_i$ in our list (where $1\le{}i\le{}n$) we have:
\[P=(p_1p_2p_3\dots{}p_{i-1}p_{i+1}\dots{}p_n)p_i + 1\]
which demonstrates (by the Euclidean Division Theorem) that there is always a remainder
of 1 when you divide $P$ by any of the primes in the list.

Therefore either $P$ itself is prime, or if $P$ is composite then there is another prime that is not on the list that divides $P$ (leaving remainder zero).

In either case, we have demonstrated that there must be a prime that is not on our list, so the assumption that we can make such a finite list is false.
