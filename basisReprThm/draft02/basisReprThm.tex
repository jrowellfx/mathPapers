...
\hspace{3em}$n=d_0b^0=1\cdot{}b^0=1\cdot{}1=1$

So since $d_0<b$ for all $b$ and $d_0\ne0$ (it's important to note that $k=0$ here),
then we've proven the base case, in other words
the integer one represents itself in all bases.
\end{jprIn}
...
In base-ten $d_0$ can be at most eight, etc.\footnote{Please read the following bidirectional arrow symbol $\Leftrightarrow$ as ``if and only if'' - it's like a logical ``equals'' sign}
%\footnote{Use
...
change being the integer associated with $b^0$, but since:
\begin{alignat*}{3}
  &&d_0
...
We're going to examine two cases within ``Case 2" here.  The first is
when ALL the digits of $n$ are equal to $(b-1)$ (eg. $n=999999$ in base-ten).
The second being when NOT ALL of the digits are equal to $(b-1)$, but at least $d_0$ is,
and possibly $b_1, b_2,\dots{}$ for some number of
the rightmost digits (eg. $n=782999$ in base-ten).

Case 2a) ALL of the digits of $n$ are equal to $(b-1)$.
\begin{jprIn}
We're going to make use of the result that we proved in our ``Intermission: Extra Exercise"!
Recall in the proof we showed that:
