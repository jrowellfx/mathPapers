...
%Let's look at a general description for just one digit of $n$.
%
%It's important to remember that we only allow $n$'s digits
%to take on the values from 0 to at most 9 - no other values are allowed.
...
mean becomes clear if I write it out like this:
...
%say $10^i$  (where $i$ is 0, 1, 2, 3, etc.),
...
$0\le{}d\le{}9$ as ``zero is less-than-or-equal-to dee which is less-than-or-equal-to ten"
...
Say $d$ represents the ``hundreds'' digit of $n$. So in our example number 92507 let's substitute
the ``5'' with $d$ - like this $92d07$.  So, $d$ contributes this much to the value of $n$.

, so the power-of-ten in question here is $10^2=100$ and if $0\le{}d<10$, then
our number could be


\[d\cdot10^2\text{, where }d\text{ is an integer such that }0\le{}d<10\]
...
are nonnegative integers less than ten.
