...
It would be helpful to have a name, or label, to stand in for any 
...
To review: each digit 9, 2, 5, 0 and 7 are 
associated with a specific power-of-ten.  The rightmost units digit, or least-significant-digit 7 is multiplied by 1
(written as $10^0$ for consistency); Shifting one digit to the left we find a 0 which is multiplied by $10^1$;
Another digit to the left find a 5 which is multiplied by $10^2$, etc\dots{}
...
Specifically, 9 are the most lots of any given
power-of-ten we are allowed to use in our base-ten representation of $n$, otherwise we could have made a pile of the next higher power-of-ten.
...
Let's specify this restriction, along with the expression:
\[n=d_3{\cdot}10^3+d_2{\cdot}10^2+d_1{\cdot}10^1+d_0{\cdot}10^0\text{, where }0\le{}d_{\{0,1,2,3\}}\le{}9\text{, and }d_3\ne0\]
I just made up the notation $d_{\{0,1,2,3\}}$ which I'm hoping coveys the information that each $d$ lies between 0 and 9 inclusive. A more 
conventional way to express this idea is to let another term, like $i$ for ``index'', stand in for the subscript then specify what values the subscript can assume.
Like this:
\[n=d_3{\cdot}10^3+d_2{\cdot}10^2+d_1{\cdot}10^1+d_0{\cdot}10^0\text{, where }0\le{}d_i\le{}9\text{, and }0\le{}i\le{}3\text{, furthermore }d_3\ne0\]
In other words, 
\begin{center}
\begin{tabular}{r l}
