...
We can show that by dividing our integer $n$ by $b$, then dividing that
...
Then we'll use another approach to prove that each such representation is unique.

A helpful way to think about how we're going
to use the
Principle of Mathematical Induction 
is to imagine
an arbitrarily long odometer in your car.
Then it's easy to imagine that for ANY number that
could be displayed on it's dials, if we drove another mile then
it would always generate another valid number.

We need to take care of a few cases algebraically to match the kinds
of things you see in the odometer:

i) the units digit is less than 9, so driving one more mile only spins
that first dial to the next number.\\
ii) The units digit, and perhaps several to the left of it, are all 9's,
so driving one more mile will spin all those 9's at once back to zero,
at the same time adding one more to the first non-nine dial by spinning it to the next
higher number.\\
iii) All the digits from the units digit up are 9's until we get to a zero,
so the next mile driven is going to 
change that first zero dial to a 1 while spinning all the nines back to zero.
In this case the number of digits will have been extended by one.
