...
in terms of powers of ten: $46307=4\times10^4+6\times10^3+3\times10^2+0\times10^1+7\times10^0$.
...
``Existence of additive and multiplicative identities'': There are numbers 0 and 1 such that, $a+0=a$ and $a\times1=a$ and
``The existence of additive inverses'': $a+(-a)=0$. Note: in general integers do NOT have multiplicative inverses 
...
The reason it's safe to think this way is because we 
spell out exactly what it means to write a number in base-ten with 
a theorem, then we show that our theorem \emph{must} be true with a ``proof''.
A proof
...
So let's see if we can formulate a theorem about what base-ten means
in a precise and economical way that says everything (but no more) about it
so that we may use it with impunity in talking about the more abstract set of integers.
Here's what the theorem might look like:

\emph{Base-Ten Theorem (wordy)}
\begin{jprIn}
Every positive integer can be written uniquely as a
sum of multiples of successive powers of ten, such that any given multiple is no greater than nine, but can be as low as zero.
We call each of these multiples a ``digit'' and write out each integer by listing its digits, starting on the left with the
digit that corresponds to the multiple of the highest power of ten, successively down to the digit that represents the multiple
of the zeroth power of ten. We note that we can also write the number zero in this manner by letting all the digits be zero.
\end{jprIn}

This says what we want to say, but it's a little hard to visualize - this is where Algebra comes in very handy.
...
The integer ten is special in this theorem, but we only have symbols for the integers zero through nine, so we need to make
up a symbol to stand in for our integer ten.  We could say something like ``Let $T$ represent the integer ten'', or
we could cheat a little, and define the symbol for ten to be ``$1\hspace{-0.2em}0$'' where we kind of squish the
