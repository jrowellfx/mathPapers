...
Let's take a moment to acknowledge why we want to let people know that $d$ is an integer.
...
but if someone were to come upon our statement in the wild how would they know that
the $d$ in ``$d\cdot{}10^2, 0\le{}d\le9$'' isn't a fraction like $\frac{22}{7}$, or the value of $\pi$ or something weird like that?
Well they wouldn't unless we told them - so to be extra careful and make everything all nice and sparkling clear
we tell them exactly what $d$ is, after all, it doesn't cost us much.
...
\end{align*}
Ok, hold on a minute - that's getting a little cumbersome and takes up way too much space,
plus did  you notice how we slipped in that different range for $d_4$?  No? I'm not surprised,
that sort of detail that $d_4$ can't be zero can easily get lost in the wave of stuff coming at you.

A common notational convention in situations like this
...
0\le{}d_i\le{}9
\text{ where }
d_i\text{ is an integer for all }i\text{ in }0\le{}i\le{}4,\\
& \text{furthermore }d_4\ne0
\end{align*}
%It's still a little wordy. Mathematics uses a lot phrases and ideas over and over, so it's come
%up with some useful notation for these oft-used statements.
%They are especially useful when the idea
%isn't that central but just needs to be acknowledged, like saying our $d_i$ terms are integers.
%
%Recall back on page-one we introduced the symbol $\mathbb{Z}$ to represent
%the set of integers.
%We can think of all numbers in mathematics
%belonging to a set of one sort or another.
%We introduce new terms by saying, for example, ``$d$ is a member of the set of integers'',
%
%or $d$ is an element of $\mathbb{Z}$.
%We have a symbol that means ``is an element of'' which looks like this ``$\in$''.
%So we can shorten ``$d$ is an element of $\mathbb{Z}$'' to ``$d\in\mathbb{Z}$''.
%
...
such that $0\le{}d_i\le{}9$, for all $i$ where $0\le{}i\le{}k$, furthermore $d_k\ne0$.
...
such that $0\le{}d_i\le{}1$, for all $i$ where $0\le{}i\le{}k$, furthermore $d_k\ne0$.
...
such that $0\le{}d_i\le{}(b-1)$, for all $i$ where $0\le{}i\le{}k$, furthermore $d_k\ne0$.
