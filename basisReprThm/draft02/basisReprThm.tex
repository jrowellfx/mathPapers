...
Suppose\footnote{The wording of this definition of ``The Principle of Mathematical Induction'' is borrowed from ``Calculus'' by Michael Spivak - an fabulous introductory textbook to Analysis.} that $P(n)$ means that the property $P$ holds
...
Base case:
\begin{jprIn}
Since $n=0$ is a slightly special case in the theorem, then lets look at both $n=0$ and $n=1$ to start our base case.
...
This case is
quite easy to deal with, so let's quickly dispense with it.
\begin{align*}
n &= d_kb^k+d_{k-1}b^{k-1}+\dots+d_2b^2+d_1b^1+d_0b^0\\
&\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\text{if and only if,}\\
...
if $n=69412999$,
then $j=3$,
since $10^3$ is the lowest power of 10 such that its digit $d_3$ is less than 9 (it's 2).
\begin{align*}
...
Let's attend to this remaining situation.

Suppose $d_i=(b-1)$ for all $0\le{}i\le{}k$, then let $d_{k+1}=0$ and $j=k+1$.

So picking up at equation \eqref{eqnA} with our new terms we have:
\begin{align*}
n+1 &=(d_j+1)b^j\\
&=(d_{k+1}+1)b^{k+1}\\
&=(0+1)b^{k+1}\\
