...
Written\footnote{recall the ``bedmas'' convention for ``order of operations'' when we have
a string of numbers and operations like multiplication, addition and exponents etc.; that is, } in terms of powers of ten: $46307=4\times10^4+6\times10^3+3\times10^2+0\times10^1+7\times10^0$.
...
that our theorem would be false is if the axioms themselves are false\footnote{The axioms: For every integer $a,b\text{ and }c$:
...
``The existence of additive inverses'': $a+(-a)=0$. Note: in general integers do NOT have multiplicative inverses (except 1 and -1) 
...
Let's start. We want to talk about each and every whole number in the
same way, so let's give this whole-number a name.  Let's use $n$, for \emph{number}.

We want to be able to say that $n$ can be represented by a string of digits,
where the value of each digit can be zero or one, or two, or \dots{} up to one-less-than-ten.
We don't want to say specifically what the digit is, just that each digit has such a value.

The way we are using $n$ (for number)
let's use a $d$ for digits.  Hold on, we probably need more than one digit to represent $n$, in fact we have no idea how many digits we might need!
We need a list of digits for each $n$.
...
Let $n$ be a positive integer, and let ``$1\hspace{-0.19em}0$'' represent the integer ten.\\
Then every $n$ can be uniquely expressed as follows:

\hspace{3em}$n=d_k{1\hspace{-0.19em}0}^k+d_{k-1}{1\hspace{-0.19em}0}^{k-1}+\dots+d_2{1\hspace{-0.19em}0}^2+d_1{1\hspace{-0.19em}0}^1+d_0{1\hspace{-0.19em}0}^0$

where $d_k\ne0$ and $0 \le d_i < {1\hspace{-0.19em}0}$ for each $d_i$.
