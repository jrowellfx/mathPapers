...
Actually any number raised to the 0\textsuperscript{th} power~is~1.\footnote{Proof: Since $a^{b+c}=a^ba^c$ consider when $c=0$; that is,
...
Written\footnote{recall the ``bedmas'' convention when we have
a string of numbers being added and multiplied together. ``bedmas'' reminds us to first apply brackets, then exponents,
division, multiplication, addition then finally subtraction when trying to resolve an expression. Note that ``bedmas'' is just a convention, it's strictly adhered to but not
a necessity in any way - aliens might use ``beasdm'' and their math would be just as valid as ours.}
in terms of powers of ten: $92507=9\times10^4+2\times10^3+5\times10^2+0\times10^1+7\times10^0$.
...
the digits can be expressed in a similar matter.
...
Identities: There are numbers 0 and 1 such that, $a+0=a$ and $a\times1=a$ and
Additive Inverse: $a+(-a)=0$. Note: in general integers do NOT have multiplicative inverses 
that are also integers (eg. $\frac{1}{2}$ is the multiplicative inverse of 2 because $\frac{1}{2} \times 2=1$ but  $\frac{1}{2}$ is not an integer!)}.
...
that if we add one, the nines roll over to zero, and 1 gets added to the 4, like an odometer in your car,
getting us to the very next integer which is 500.  We know this always works no matter what number we add 1 to.

Exactly - our intuition is good. Let's write it down as something that has to be
in our theorem. We would say something like.
...
Something else that's important, and something we know intuitively, is that any number written
in base-ten represents only ONE integer. It's almost stupid to spell it out, but if we were to count out
four piles of one-hundred apples, then nine piles of ten apples, then nine additional apples, then
...
Then if we finished by counting out nine remaining single apples
then we know we have 499 apples, and there is NO other way to split this big pile up
in ANY other way if we follow this rule.  In other words, each integer is representable by only ONE base-ten number.
...
is associated with a specific power of ten.  The rightmost units, or least-significant digit (7) is multiplied by 1
or better yet $10^0$; the next digit to the left (0) is multiplied by $10^1$, etc\dots{}
until we finally come to the leftmost or most-significant digit (9) which is multiplied by $10^4$, THEN we add them all together.
...
It's critical to recognize that we only allow our digits to be zero through nine, no other values are allowed.
Specifically, nine is the maximum number of lots of any given
...
For example, if we had nine piles of ten apples each, but still had exactly ten apples left over, we would simply add that ten
to the other nine piles of ten and count it as one pile of a hundred.  That's the rule - we are only allowed
at most nine lots of a certain ``power of ten'' sized pile of apples.

Let's call $d$ a ``digit'' of our number such that the $d$ represents how many lots of some power of ten we have,
then $d$ must be between zero and nine inclusive, and the contribution of that digit to the integer would be this:
\[d\cdot10^i\text{, where }0\le{}d\le{}9\text{ and }i\ge{}0\]
In other words, for some power of ten (that's the $i$ which is any number greater than or equal to zero) we have $d$ lots of it. Note that $d$
could be zero, which means we don't have ANY lots of this particular power of ten, all the way up to the maximum
number of lots of it being nine.

Our expression for the integer should only have one digit for each power of ten, in other words we wouldn't want to have
something like this.
...
Also, we know that a number is composed of possibly many ``digits'' times various powers of ten, all added together.
See our example for the number 92507 a few pages back, to remind you.

Here's what the theorem might look like generalizing from our example of the integer 92507 on the previous page:
...
\[n=d_0b^0=1\cdot{}b^0=1\times{}1=1\]
