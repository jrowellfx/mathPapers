...
Our number 92507 only has five digits and it's biggest power of ten is $10^4$, but there's
no limit on how big a power-of-ten could be involved in our expression. Look at ``a trillion and one'', i.e.; 1,000,000,000,001 which can be expressed as:
%\begin{small}
\[1{\times{}}10^{12}+0{\times{}}10^{11}+0{\times{}}10^{10}+0{\times{}}10^{9}+\cdots{}+0{\times{}}10^{3}+0{\times{}}10^{2}+0{\times{}}10^{1}+1{\times{}}10^{0}\]
%\end{small}
Let's look at another one - we don't have a word for this next number, and I don't have enough room on the page to write it out, but this number would be twenty-thousand-and-one digits long, a
``7'' followed by twenty-thousand zeros:
%\begin{small}
\[7{\times{}}10^{20000}+0{\times{}}10^{19999}+\cdots{}+0{\times{}}10^{3}+0{\times{}}10^{2}+0{\times{}}10^{1}+0{\times{}}10^{0}\]
%\end{small}
%Even though we can think about the number 4100 as ``forty-one hundred'' - we don't
%allow ourselves this luxury. 
Claim: 
By stringing together \emph{as~long~a~list~of~digits} as pleases us,
we can uniquely represent every positive integer we want no matter how big it is.
...
So let's flesh out our ``claim'' above into a more precise ``Base-Ten Representation Theorem''.
Then we prove our theorem so that we may confidently go forward using base-ten with
...
4 piles of 100-apples-each-pile, then 9 piles of 10-apples-each-pile, then count out 9 additional apples, \emph{then}
scoop them all into a big pile that
we'd always get the exact same size big-pile-of-apples.

It goes the other way too. If we were handed the aforementioned big pile of apples we could start counting out
piles of 100. We'd try to make as many piles of 100 as we could, and we'd find that we'd have 4 piles of 100 before
we couldn't make another such pile.
Then we would start counting out piles of 10 with the remaining apples.
After we made as many piles of ten as we could out of those remaining apples, we would discover that we'd have 9 such piles of ten
with 9 apples left over, in other words 499 apples! There is NO other way to divvy up this big pile of apples
if we follow this procedure.  In other words, each integer is represented by only ONE base-ten number.
...
Thinking about it another way, if we allowed a digit to take on a value that was greater than nine - it would affect
MORE than just the power of ten that it's associated to. Say we allowed digits to be as big as eleven and
that we use this ancient mystical symbol for eleven that I just made up which looks like this {1\hspace{-0.6em}\raisebox{0.05em}{$\sim$}}.
...
So $d$ represents how many lots of some power-of-ten, say $10^i$  ($i$ is 0, 1, 2, 3, etc.),
...
$91d32$, so the power of ten in question here is $10^2=100$ and if $0\le{}d\le9$, then
our number could be 91032, 91132, 91232, 91332, 91432, 91532, 91632, 91732, 91832 or 91932.}:
\[d\cdot10^i\text{, where }d\text{ is an integer such that }0\le{}d\le{}9\text{ and }i\ge{}0\]
In other words, for some power-of-ten (the $10^i$) we have $d$ lots of it. Note that $d$
...
However, if someone were to come upon the following statement in the wild:
\[d\cdot10^i\text{, such that }0\le{}d\le{}9\]
\dots{}how would they know that $d$ isn't a fraction like $\frac{22}{7}$, or the value of $\pi$ or something weird like that?
Answer: They wouldn't know -  which is why it's important to spell it out.
\bigskip
...
\dots{}meaning the value of the \emph{actual} digit associated with $10^2$ is eleven, which as we examined above
has nasty consequences, and thus 
violates our rule of keeping the value of the digit in the range of 0 to 9.
\bigskip
...
Why? Because if $d_4$ were zero then $n$ would only be a four-digit number, or perhaps a three-digit number, or only two-digits etc.
...
So there we have it, we found our expression for being able to express each positive integer, let's use
...
to prove the Base-Ten Representation Theorem - we're going to keep digging, and eventually prove something
...
Claim: 
The inhabitants of Planet-Claire can string together \emph{as~long~a~list~of~binary-digits} as pleases them,
in order to uniquely represent every positive integer they want no matter how big it is.
...
where $0\le{}d_i\le{}1$ for all $i$ in $\{0,1,2,\dots{},k\}$ and $d_k\ne0$.
...
where $0\le{}d_i\le{}(b-1)$ for all $i$ in $\{0,1,2,\dots{},k\}$ and $d_k\ne0$.
