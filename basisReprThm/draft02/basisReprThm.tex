...
this kind of an algebraic expression can be done for EVERY string of decimal digits. It's the key to 
...
it's just some number from 0 to 9 times a power-of-ten like any of the other digits.
Including $10^0$ in the expression adds a kind of beauty because it shows us that all
the digits can be treated with the same underlying idea.
...
How do we \emph{know} that we can create ALL the positive
...
it's safe to think that each
integer has its own unique base-ten label.
But at least once in the past, someone has to take the time to
spell out exactly what it means to represent a number in base-ten. This kind of a statement
is called a theorem\footnote{I can't find where
the first such theorem was stated, I think it's quite recent.}. Then that theorem must be proven to be true with a series of arguments that logically connects it
...
The rest of the paper is about taking this journey. In fact we're going to end up generalizing upon our
...
\break
So let's start our journey. We intuitively know that counting with
