...
paper is going to represent a non-negative integer, or \emph{very} occasionally a rational-number.
...
of the two sets are the same. The approach
taken will establish that each element in one set must be equal to exactly one element in the other set.
...
\(n \ge{} 2\).
% , and this sum is uniquely associated with \(\frac{p}{q}\).
...
% Also, I think it is easy to demonstrate the following,
% where \(\mathbb{Q}^+\) is the set of all non-negative rational numbers:
% 
% {\small
% \begin{gather*}
% \mathbb{Q}^+ = \{1\} \cup \bigcup\limits_{k=2}^{\infty} \{k\} \cup \mathcal{S}_k,
% \end{gather*}
% }
% 
% which suggests an easy way to show that \(\mathbb{Q}^+\) is countably infinite.
% 
\end{document}
