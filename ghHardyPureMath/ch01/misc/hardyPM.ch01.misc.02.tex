...
\newcommand{\jprVersion}{v63.3} % version
...
\hypersetup{
    pdfauthor={James Philip Rowell},
    pdfcopyright={Copyright 2018-2025 by James Philip Rowell. All rights reserved.},
    pdfpagemode=UseNone
}
\usepackage{lipsum}
...
The theorem feels like what the `\href {https://github.com/jrowellfx/mathPapers/blob/master/basisReprThm/basisReprThm.pdf}
{basis-representation-theorem}'
is for integers, but for rational numbers \dots{} beautiful!

As a bonus, we can use the following theorem to prove that the famous mathematical constant \(e\) is irrational with almost no extra effort!
...
+ \frac{k\cdot{}(k-1)\cdot{}\cdot{}\cdot{}4\cdot{}a_3}{k!}
+ \dots{}
...
\subsubsection*{Bonus: Proof that \(e\) is irrational.}

It is well known that the mathematical constant \(e\) can be expressed as follows:
...
\begin{center}
\begin{equation*}
e = 2 + \frac{1}{2!} + \frac{1}{3!} + \frac{1}{4!} + \frac{1}{5!} + \dots{}. 
\end{equation*}
\end{center}
\egroup
}
...
Its
easy to see that its fractional-part is not in \(\mathcal{S}\)
because we can't point to any particular \(\mathcal{S}_k\) that it would be a member of.
