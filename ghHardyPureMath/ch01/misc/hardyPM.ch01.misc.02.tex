...
Any positive rational number\footnote{Amost every variable,
or constant (eg. \(a_1, a_k, m, n, i, p, q\)) in this
paper is going to represent a non-negative integer, or \emph{very} occasionally a rational-number directly.
...
The ideas above are behind the proof that follows.
...
By showing that the smallest and largest numbers in \(\mathcal{S}_k\)
are the same as those in \(\mathcal{F}_k\), then because each element of \(\mathcal{S}_k\)
is forced to be one of the elements in \(\mathcal{F}_k\) (because there are no other
possibilities) then this establishes equality between the two sets.
