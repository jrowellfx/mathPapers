...
Even though we use different symbols, the ancient Romans and we
...
%
%Also since negative numbers are unique (they pair one-to-one with
...
\end{alignat*}
QED
\section*{Induction Proof of the Basis Representation Theorem}
Using the
...
the the brochure our new car has a magic-odometer.
We notice as we
pull out of the dealership parking lot that
the odometer is only one digit wide.
That one and only digit shows a ``1", I guess the factory was only one mile away.
...
Congratulations if you've never tried your hand a a proof,
nor perhaps even read one before - you made it to the end.

My hope with the paper is that by
taking a familiar subject like counting \`a la Sesame Street and applying rigorous mathematical reasoning,
...
I feel that not being able to enjoy mathematics is as tragic as someone who is blind who can't enjoy the MoMA, or
