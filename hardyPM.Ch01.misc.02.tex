...
every lower power of ten each times 9.  For example, 1000 is bigger than 999.
...
Furthermore, examining any single term on the right-side of \eqref{eqn1}, say
...
\(a \in \mathcal{F}_k\). Similarly for \(a \in \mathcal{S}_\infty\).
...
By induction it is simple to conclude that
\(\mathcal{F}_k
...
\subset \mathcal{F}_n
\), for all \(n > k\),
and ultimately \(\mathcal{F}_k \subset \mathcal{F}_{\infty}\) for all \(k \ge{} 2\).
...
\(0\le{}\frac{p}{k}<1\)
then \(\frac{p}{k} \in{} \mathcal{S}_k\) because,
...
\(\mathcal{S}_k = \mathcal{F}_k\) for all \(k \ge 2\), then \(\frac{p}{q} \in \mathcal{S}_\infty\).

From this we can conclude that there is a unique sequence of variables 
