...
\section*{Factorial Representation Theorem\footnote{...it's not named in the text, so I named the theorem.}}
...
At first glance it wasn't remotely obvious to me how I'd go about calculating
...
for a given rational number \(\frac{p}{q}\), (it's kinda like doing long-division)
...
An obvious patter emerged!
It seemed to be the case
that if we assign the largest possible values to the variables,
from \(a_2\) up to say \(a_k\) (and all subsequent variables
set to zero) we get this rational number, \(\frac{k!-1}{k!}\).
...
\begin{center}
\begin{equation*}
\frac{1}{2!} + \frac{2}{3!} + \dots{} + \frac{k-1}{k!} = \frac{k!-1}{k!},\quad \text{for integer } k \ge{} 2
\end{equation*}
\end{center}
\egroup
}
\section*{Proof of Lemma 1 (make smaller)}

This equality is straighforward to demonstrate by induction, since \(\frac{1}{2!} = \frac{2!-1}{2!}\) and, 
{\scriptsize
\begin{alignat*}{1}
...
\dots{}thus establishing lemma 1 for all values of \(k \ge 2\).\quad QED.
...
\begin{alignat*}{2}
  ...
  =\  &\frac{k!-1}{k!} - \frac{i!-1}{i!} \text{\quad \quad \quad (by Lemma 1)}\\
...
Note JPR : line up by equals sign
...
First suppose that \(a_i\ne{}b_i\) is the first such pair of variables
that differ. In other words, \(a_2=b_2, a_3=b_3, \dots{}, a_{i-1}=b_{i-1}, a_i\ne{}b_i\).
Also, without loss of generality we can assume that \(a_i > b_i\), and because
of the equality of the two different representations for \(\frac{p}{q}\) we can write,
...
(JPR NOTE: possibly add more terms to make pattern clear.)

But \(a_i - b_i \ge{} 1\), so \(\frac{a_i - b_i}{i!} \ge{} \frac{1}{i!}\).

Also, (Rewrite by starting out with footnote "hence" etc.)
...
and all the \(a\)'s be zero will produce the largest numerators in each
term of the sum; any other possibility will result in a smaller term for the sum.}.

Because lemma 2 tells us that \(\frac{1}{i!} > \frac{i}{(i+1)!} + \dots{} + \frac{k-1}{k!}\)
...
But we had also deduced
that \(\frac{a_i - b_i}{i!} = \frac{b_{i+1} - a_{i+1}}{(i+1)!} + \dots{} + \frac{b_k - a_k}{k!}\)
which contradicts the strict inequality above. (Rewrite using label to above equality).
...
Therefore any assignment of values to the variables of
\(\frac{a_2}{2!} + \frac{a_3}{3!} + \dots{} + \frac{a_k}{k!}\)
is unique.
...
creates a unique member of the set, then the size of \(\mathcal{S}_k\) is \(k!\,\), which
...
Furthermore the corollary is a direct result of the fact that there is a one-to-one mapping
between the two sets.
QED

\section*{Lemma 4}

Any non-negative rational number \emph{less than one},
...
{\small
\begin{center}
\(0\le{}a_2<2,\quad 0\le{}a_3<3,\quad \dots{},\quad 0\le{}a_k<k\)
\end{center}
}

\section*{Proof Lemma 4}

If \(\frac{p}{q}\) is a rational number where
\(0\le{}\frac{p}{q}<1\) and \(2\le{}q\le{k}\)
...
\dots{},
\frac{k-2}{k},
\frac{k-1}{k}\).}, then \(\frac{p}{q} \in{} \mathcal{S}_k\) because,
...
Therefore \(\frac{p}{q}\) is an element of \(\mathcal{F}_k\), hence an element of \(\mathcal{S}_k\).
In other words, all non-negative rational numbers \(\frac{p}{q}\) with denominators \(q\) from 2 up to \(k\)
where \(0\le{}\frac{p}{q}<1\) 
are uniquely represented by the expression,
...
where \(a_2, a_3, \dots{}, a_k\) are integers, and
\(0\le{}a_2<2,\quad 0\le{}a_3<3,\quad \dots{},\quad 0\le{}a_k<k\)

If we fix \(q\) and let \(k\) go to infinity then it's clear that we can make a set \(\mathcal{S}_\infty\) that contains
all rational numbers between zero and one.
For example \(\mathcal{S}_{k+1}\) would ALSO contain all rational numbers with denominator \((k+1)\) that
are between zero and one, 
and \(\mathcal{S}_{k+2}\) would ALSO contain all fractions with denominator \((k+2)\), 
that are between zero and one, etc. until you get them all.
Since \(\frac{p}{q}\) is uniquely represented by
our expression within \(\mathcal{S}_n\) for ALL values of \(n\ge{}k\)
then it is unique for all rational numbers between zero and one.

QED

Alternate observation: Another way to see this is to look at what happens to the 
expression for \(\frac{p}{q} \in \mathcal{S}_k\) when we let \(\mathcal{S}_k\) increase in size.
...
Therefore, \(\frac{p}{q} \in{} \mathcal{S}_n\) for all \(n \ge{} k\).

\break
\section*{Factorial Representation Theorem (restated)}
...
Therefore to express any positive rational number in the form of the theorem, first apply the
...
by lemma 4 we know that the sum \(\frac{p}{q} = \frac{a_2}{2!} + \frac{a_3}{3!} + \dots{} + \frac{a_k}{k!}\) is
uniquely associated
with \(\frac{p}{q}\)
...
To meet the precise requirements of the theorem, we 
suppose that if \(\frac{p}{q} \in{} \mathcal{S}_n\) for some \(n \ge{} q\)
that we take
for the variables \(a_2, a_3, \dots{}, a_k\) in the sum for \(\frac{p}{q} \in{} \mathcal{S}_n\)
...
\frac{ 5}{6}
\}
\), which demonstrates the claim above that \(\mathcal{S}_3\) contains \(\frac{p}{6}\), where \(0 \le{} \frac{p}{6} < 1\).
