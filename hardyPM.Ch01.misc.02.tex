...
of the theorem then the integer part, \(i\), will be the \(a_1\) term,
and the remainder of the expression should be the rational part, \(\frac{p}{q}\), such that,
...
going to simplifiy the task too.
...
that if we assign the biggest possible values to the coefficients,
from \(a_2\) up to say \(a_k\) we get this rational number, \(\frac{k!-1}{k!}\). This turned out to be
...
So for a given sum \(\)
This seems pretty useful because for a given 
to cover the range for our \(\frac{p}{q}\) part of our specific rational
number where \(0 \le{} \frac{p}{q} < 1\).
...
Futhermore we've shown above that there is a unique sequence of integers \(a_2, a_3, \dots{} a_k\),
...
are identicaal and there are k! possible combinations of 
