...
Furthermore the corollary was shown during the course of the proof.  QED
...
In other words, all non-negative rational numbers with denominators from 2 up to \(k\) that are less than 1
...
If we fix q and let k go to infinity... then we have result for all possible denominators and.

But furthermore,  

JPR here. fix...

...this unique representation is also the same for all rational numbers greater than \(k\) because
...
are members is the set \(\mathcal{S}_k\) where \(k\) is the smallest value for which
\(q\) divides \(k!\).
