...
I know that any positive rational number\footnote{Every variable,
...
then later prove the full theorem by adding back the \(a_1\) to get all positive rational numbers.

Including zero (that is, not just \emph{positive} rational numbers) was also
...
(where
\(0 \le{} \frac{p}{q} < 1\)).
But after playing around for a while, and finally figuring out
...
Furthermore
I could make
the smallest positive number, \(\frac{1}{k!}\), by
letting all the variables be zero except for \(a_k = 1\).
Then I considered what happens to the variables by
continually adding \(\frac{1}{k!}\), repeatedly, over and over.

With various assignments of values to
the variables \(a_2, a_3, \dots{}, a_k\), I could
generate the smallest number, \(\frac{0}{k!}\), as well as the largest,
\(\frac{k!-1}{k!}\).
...
\enspace \dots{},\enspace \frac{k!-1}{k!}\}\). So perhaps forming a set
out of all possible assignments of values to the expression would give us the same set?

Another clue for me was the insight that in order to be able to represent
any fraction with a prime number in the denominator, then the sum in our
expression would have to have a non-zero term with that prime number as a factorial
in its denominator. In other words, for a fraction \(\frac{}{}\)

With a little algebraic tinkering we can see that
this set contains \emph{all} of the rational numbers between zero and one with
denominators from 2 up to \(k\), so if we let \(k\) grow without bound
then we should get a set that contains \emph{all} the rational numbers between zero and one.
...
Suppose \(a \in \mathcal{S}_k\). Then \(a = \frac{a_2}{2!} + \frac{a_3}{3!} + \dots{} + \frac{a_k}{k!}\),
for some a2, a3, ... ak where
\(k \ge{} 2\), and \(0\! \le{}\! a_2\! <\! 2,\enspace
0\! \le{}\! a_3\! <\! 3,\enspace
\dots{},\enspace 0\! \le{}\! a_k\! <\! k\). This is called the ``factorial-representation of \(a\)''.
...
To show
that the set \(\mathcal{S}_k\ = \mathcal{F}_k\), it suffices to show that if \(a \in \mathcal{S}_k\)
then a is in  Sk
% \(0\le{}a<1\) and 
\(\frac{a}{b} = \frac{p}{k!}\) for some \(p\),
and that the size of \(\mathcal{S}_k\) is the same as \(\mathcal{F}_k\).

JPR NOTE: Magnitude and subset - that's all.
...
produces a \emph{unique} member of the set \(\mathcal{S}_k\)\footnote{\dots{} and a good thing too
...
% which you may recall means \(a \in \mathcal{F}_k \Leftrightarrow a \in \mathcal{S}_k\).
...
Finally,
\(\mathbb{Q}_{01}\) is the set of ALL non-negative rational numbers less than one, 
where \(p\) and \(q\) are co-prime. We specify the co-prime condition
to insure uniqueness members of the set (otherwise the set isn't well defined).
Also we want zero, so we have to toss it back into the set.
% \footnote{By definition sets must contain unique members.}.
I'm not going to prove it here, but \(\mathbb{Q}_{01}\) contains ALL the
rational numbers between zero and one; I'm taking this as established.
