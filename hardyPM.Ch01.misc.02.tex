...
we get this rational number, \(\frac{k!-1}{k!}\).
...
\bigskip
The following Lemma 2 captures an idea that is best described by analogy to the basis representation
...
9 times each lower power of ten.  For example, 1000 is bigger than 999.
...
\begin{alignat*}{2}
  ...
  =\  &\frac{k!-1}{k!} - \frac{i!-1}{i!} \text{\qquad \qquad \qquad (by Lemma 1)}\\
...
that differ. In other words, \newline \(a_2=b_2, a_3=b_3, \dots{}, a_{i-1}=b_{i-1}, a_i\ne{}b_i\).
...
The set \(\mathcal{F}_\infty\) contains all the non-negative rational numbers less than one.
...
By induction is simple to conclude that
\(\mathcal{F}_k
\subset \mathcal{F}_{k+1}
\subset \mathcal{F}_{k+2}
\subset \dots{}
\subset \mathcal{F}_n
\), for all \(n > k\),
and ultimately \(\mathcal{F}_k \subset \mathcal{F}_{\infty}\) for all \(k\).
