...
If \(a \in \mathcal{S}_k\), then the sum \(\frac{a_2}{2!} + \frac{a_3}{3!} + \dots{} + \frac{a_k}{k!}\),
where
...
to get unique members of the set, otherwise the set isn't well defined\footnote{By definition sets
...
However to shed more light on the matter, once a particular rational number \(\frac{p}{k!}\)
...
ergo \(\frac{p}{k} \in \mathcal{F}\).


The converse is also true; That is; \(a \in \mathcal{F}\), implies \(a \in \mathbb{Q}_{01}\).
...why? - need to show m/n! = a/b s.t. gcd(a, b) = 1

hence, a in F iff a in Q01, hence
...
Because of Lemma 3 we know that the sets
\(\mathcal{F}_k\) and \(\mathcal{S}_k\) are equal and interchangeable, so clearly S = F = Q, hence
