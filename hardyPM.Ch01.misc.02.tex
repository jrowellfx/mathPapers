...
Also, if we assign zeros to all the coefficients then naturally we get \(\frac{0}{k!}\), plus
...
\(a_i\) terms change as you keep incrementing by \(\frac{1}{k!}\).
...
and that the size of \(\mathcal{S}_k\) is \(k!\, \).
...
we can string all our inequalities together to get the following.
...
which is in contradiction with the above strict inequality.
Therefore our assumption that there can be a second set
of coefficients to produce the same rational number \(\frac{p}{k!}\) must be false.
...
Thanks to Euclid we know that for all integers \(h\ge{}0\) and \(q>0\),
...
\setlength{\abovedisplayskip}{0pt}       %% effective inside the group
\begin{alignat*}{3}
&&h = i\cdot{}q + p\ &;\quad 0\le{}p<q \\
&\Leftrightarrow\quad &\frac{h}{q} = i + \frac{p}{q}\ &;\quad 0\le{}\frac{p}{q}<1
\end{alignat*}
\egroup
}\par
Which tells us that all rational numbers \(\frac{h}{q}\) can be written as
...
Euclidean Division Theorem to \(\frac{h}{q}\) and let \(a_1 = i\). If there is no fractional remainder, then
...
uniquely associated
with \(\frac{p}{q}\)
then clearly \(\frac{h}{q} = a_1 + \frac{a_2}{2!} + \frac{a_3}{3!} + \dots{} + \frac{a_k}{k!}\)
is uniquely associated with all rational numbers \(\frac{h}{q}\).

To exactly meet the requirements of the theorem, we throw away that fact that we can
represent zero, and we take
for the coefficients \(a_2, a_3, \dots{}, a_j\) in the sum for \(\frac{p}{q} \in{} \mathcal{S}_k\)
all those for which \(a_j \ne{} 0\) but \(a_{j+1} = a_{j+2} = \dots{} = a_k = 0\).
...
I believe that for a given \(q \ge 2\) then the smallest set for which rational number
