...
an assignment of integers \(a_2, a_3, \dots{}, a_k\) for a given rational number
\(\frac{p}{q}\), where
\(0 \le{} \frac{p}{q} < 1\),
let alone that it would be unique.

After playing around for a while, and finally figuring out
a way to calculate the \(a_i\) variables
...
If we fix \(q\) and let \(k\) go to infinity then it's clear that we can make a set \(\mathcal{S}_k\) that contains
all rational numbers between zero and one.
Since \(\frac{p}{q}\) is uniquely represented by
our expression within \(\mathcal{S}_k\) then it is unique for all rational numbers between zero and one.
...
To meet the precise requirements of the theorem, we throw away that fact that we can
represent zero, and suppose that if \(\frac{p}{q} \in{} \mathcal{S}_n\) for some \(n \ge{} q\)
that we take
