...
% Two sets 
% \(\mathcal{X}\) and
% \(\mathcal{Y}\) are equal if and only if,
% \(a \in \mathcal{X} \Leftrightarrow a \in \mathcal{Y}\).

We're going to establish equality by showing three properities
...
could be anywhere on the numberline
...
the numbers in \(\mathcal{S}_k\) sit on the numberline, and thus establish equality between the two sets.
...
We are not going to prove it here, but \(\mathbb{Q}_{01}\) contains ALL the
rational numbers between zero and one; we will take this as established.
...
Not surprising really, this number is \(e\). Furthermore I think we just proved that \(e\) is irrational!!
