...
looks to be playing the roll of the rational part, \(\frac{p}{q}\), where,
...
restriction allow \(a_k\) to be equal to zero so all the variables are treated the same. At the very end of the
...
the \(a_k\) variable (0, 1, 2, \dots{}, \(k\!-\!1\)), that
...
\dots{}thus establishing lemma-1 for all values of \(k \ge 2\). QED
...
For integer \(k \ge 2\), and integers \(a_2, a_3, \dots{}, a_k\),
...
Because lemma-2 tells us that \(\frac{1}{i!} > \frac{i}{(i+1)!} + \dots{} + \frac{k-1}{k!}\) then
we can string all our inequalities together like so,
...
of variables to produce the same rational number \(\frac{p}{k!}\) must be false.
Therefore any assignment of values to the variables of
\(\frac{a_2}{2!} + \frac{a_3}{3!} + \dots{} + \frac{a_k}{k!}\)
must produce a unique member of the set \(\mathcal{S}_k\).
...
Any non-negative rational number
...
Therefore \(\frac{p}{q}\) is an element of \(\mathcal{F}_k\) which is the same as the set \(\mathcal{S}_k\).
...
Which tells us that all positive rational numbers \(\frac{j}{q}\) can be written as
